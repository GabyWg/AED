...
7- Quantity of shipments by type of shipment and type of payment (By category)
8- Shipping type data with the highest amount.
0- Program exit.
Select an option: '''
    return text
...
def number_valid(min_num, max_num, sms):  # Para validar el ingreso de numeros
    n = input(sms)
    digit_flg = isdigit(n)
    while not digit_flg or (min_num > int(n) or int(n) > max_num):
        system("cls")
...
def load_data_binary_shipment(arch_name_csv,
                              arch_name_binary):  # Para crear un archivo binario en base a un archivo csv
    ...
    if arch_csv is not None:
        ...
        if arch_binary is not None:
            answer = number_valid(0, 1,
                                  f"You have shipping data loaded, if you continue they will be deleted.\n"
                                  f"0- Cancel operation.\n1- Continue.\nSelect an option: ")
            if answer == 0:
...
def load_data_keyboard_shipment(arch_name_binary):  # Para cargar datos via ingreso de teclado
    ...
    while cod_pos_flg:
        cod_pos = input('Enter the postal code: ').upper()
    ...
    ship_data = reg.Envio(cod_pos, address, id_type_shipment, waypay, cost_shipment, country_shipment,
                          province_shipment)
...
def main_arch(name_arch, mode, flg_open=True,
              arch=None):  # Funcion que abre el archivo que se le envia, al finalizar la lectura se le enviar otros
    # parametros de otra funcion para cerrar el mismo
...
def analyze_line(text_line):  # Funcion principal, analiza toda la linea de envio
    ...
    if len(text_data) == 4 and str(text_data[0].strip()) != 'codigo_postal':
        codpos_line, address_line, id_type_shipment_line, waypay_line = str(text_data[0].strip()), str(
            text_data[1].strip()), int(text_data[2]), int(text_data[3])
        ...
        ship_data = reg.Envio(codpos_line, address_line, id_type_shipment_line, waypay_line, cost_shipment,
                              country_shipment, province_shipment)
...
def analyze_codpos(cod_pos):  # Analiza el codigo postal, devuelve Pais, provincia y cargo del envio
    ...
    letter_tup = ('B', 'C', 'K', 'H', 'U', 'X', 'W', 'E', 'P', 'Y', 'L', 'F', 'M', 'N', 'Q', 'R', 'A', 'J',
                  'D', 'Z', 'S', 'G', 'V', 'T')
...
def print_count_mat(result_count):
    ...
    print("\nTotal per shipping types:\n")
    for i in range(len(count_type_shipment)):
        print(f"Shipping type {i}: {count_type_shipment[i]}")

    print("\nTotal per waypay:\n")
    for i in range(len(count_waypay)):
        print(f"Waypay {i+1}: {count_waypay[i]}")


def order_min_to_max_codpos(v_ship):
    len_vec = len(v_ship)
    div = len_vec // 2
    ...
    while div > 0:
        for i in range(div, len_vec):
            ...
            while j >= div and v_ship[j - div].cod_pos > temp.cod_pos:
                v_ship[j] = v_ship[j - div]
                j -= div
            v_ship[j] = temp
        div //= 2
...
def generate_order_array(arch_name, v_ship):
    ...
    acu = turn = 0
    while arch.tell() < tam:
        ship_data = pickle.load(arch)
        acu += ship_data.cost_shipment
        turn += 1

    if turn <= 0:
        print("No data to read file.")
    else:
        avg = acu / turn
    ...
    while arch.tell() < tam:
        ship_data = pickle.load(arch)
        if ship_data.cost_shipment > round(avg, 2):
            v_ship.append(ship_data)

    main_arch('-1', '-1', False, [arch])  # Cierra el archivo.
    
    return v_ship


def show_vector(v_ship):
    for ship in v_ship:
        print(ship)